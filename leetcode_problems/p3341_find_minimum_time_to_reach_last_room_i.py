...
def min_time_to_reach(moveTime: list[list[int]]) -> int:
    # working_sol (97.28%, 90.46%) -> (112ms, 18.14mb)  time: O(n * m * log(n * m)) | space: O(n * m)
    # Standard Dijkstra.
    time: int
    row: int
    column: int
    ...
    while que:
        time, row, column = heapq.heappop(que)
